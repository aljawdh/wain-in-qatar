...
function mapOpenMeteoCurrentToArabicSky(cur) {
  ...
  if (code === 95 || code === 96 || code === 99 || code === 97 || code === 98) return 'عواصف رعدية';
  ...
  if (code === 2 || code === 1) return 'غائم جزئياً';
  if (code === 0) {
    if (vis != null && vis < 2000 && (totalRain == null || totalRain < 0.05) && (rh == null || rh < 75)) {
      return 'مغبر';
    }
    return isDay ? 'مشمس' : 'صافي';
  }
  if (vis != null && vis < 800 && (code === 0 || code === 1 || code === 2)) return 'ضباب';
  if (vis != null && vis < 3000 && vis >= 800 && (totalRain == null || totalRain < 0.05) && (code === 0 || code === 1 || code === 2) && rh != null && rh < 70) {
    return 'مغبر';
  }
  return WEATHER_UNAVAILABLE_AR;
}
...
async function getWeatherData(station, asOfDate) {
  ...
  try {
    ...
    weatherUrl.searchParams.set('current', 'wind_speed_10m,wind_direction_10m,relative_humidity_2m,is_day,weather_code,cloud_cover,precipitation,rain,visibility');
    ...
    if (!responses[0].ok || !responses[1].ok) throw new Error('open_meteo_http');
    ...
    if (li.wind_speed_kmh == null && li.temp_c == null && li.wave_height_m == null && li.current_speed_ms == null) {
    ...
    if (ent && ent.live_inputs) {
      ...
      out.weather_status_ar = cachedAr || mapOpenMeteoCurrentToArabicSky({
...
async function fetchWeatherAndMarineInputs(station, body) {
  ...
  if (body && body.datetime) {
    ...
    if (ds.length >= 10 && /^\d{4}-\d{2}-\d{2}/.test(ds)) asOf = ds.slice(0, 10);
  }
