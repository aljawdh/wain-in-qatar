(function () {
  ...
  function buildDefaultCalibration(station) {
    ...
    var result = {
      ...
      daysCount: currentDur ? parseNumber(currentDur.days_count, 30) : 30,
      traits: {
        general: [],
  ...
  function renderFilters() {
    if (controls.durFilter) {
      controls.durFilter.innerHTML = '<option value="all">الكل</option>' + sortDurByStartDate(dururCache).map(function (dur) {
    ...
    if (controls.calibDurSelect) {
      controls.calibDurSelect.innerHTML = '<option value="">اختر الدر</option>' + sortDurByStartDate(dururCache).map(function (dur) {
        return '<option value="' + dur.id + '">' + getDurLabel(dur) + '</option>';
      }).join('');
    }
  }
  ...
  function renderCalibrationSummary() {
    ...
    var daysCount = calibration.daysCount || '--';
    ...
    controls.calibSummaryArea.innerHTML = '' +
      '<div class="info-row"><span>الدر المعاير:</span><strong>' + getDurLabel(dur) + '</strong></div>' +
      '<div class="info-row"><span>نطاق الدر:</span><strong>' + (dur ? getDurDateRange(dur) : '--') + '</strong></div>' +
      '<div class="info-row"><span>بداية معتمدة:</span><strong>' + startDate + '</strong></div>' +
      '<div class="info-row"><span>عدد الأيام:</span><strong>' + daysCount + '</strong></div>' +
  ...
  function renderTraitGroups() {
    var calibration = selectedStation ? getStationCalibration(selectedStation) : null;
    var groups = ['general', 'weather', 'marine', 'fish', 'heritage'];
    groups.forEach(function (group) {
      ...
      options.forEach(function (tag) {
        ...
        chip.type = 'button';
        chip.className = 'trait-chip' + (selected.includes(tag) ? ' active' : '');
        chip.textContent = tag;
        chip.dataset.group = group;
        chip.dataset.tag = tag;
        chip.addEventListener('click', function () {
          toggleTraitTag(group, tag);
        });
  ...
  function addTraitFromInput(group) {
    ...
    if (availableTraits[group].indexOf(tag) < 0) {
  ...
  function renderTraitOptions() {
    ...
    dururCache.forEach(function (dur) {
      availableTraits.weather = availableTraits.weather.concat(Array.isArray(dur.weather_traits) ? dur.weather_traits : []);
      availableTraits.marine = availableTraits.marine.concat(Array.isArray(dur.marine_traits) ? dur.marine_traits : []);
      availableTraits.fish = availableTraits.fish.concat(Array.isArray(dur.fish_traits) ? dur.fish_traits : []);
      if (dur.heritage_meaning) availableTraits.heritage.push(dur.heritage_meaning);
      if (dur.name) availableTraits.general.push(dur.name);
    });
    seasonEventsCache.forEach(function (event) {
      availableTraits.general = availableTraits.general.concat(event.name ? [event.name] : []);
      availableTraits.weather = availableTraits.weather.concat(Array.isArray(event.traits) ? event.traits : []);
    });
    comparisonsCache.forEach(function (record) {
      availableTraits.advice = availableTraits.advice.concat(Array.isArray(record.observed_traits) ? record.observed_traits : []);
      availableTraits.general = availableTraits.general.concat(Array.isArray(record.expected_traits) ? record.expected_traits : []);
    });
    Object.keys(availableTraits).forEach(function (group) {
      availableTraits[group] = safeUnique(availableTraits[group]);
    });
  }
    ...
    if (controls.calibDurSelect) {
      controls.calibDurSelect.addEventListener('change', function () {
        ...
        if (dur) {
          ...
          calibration.daysCount = parseNumber(dur.days_count, calibration.daysCount);
          calibrationStore[getStationId(selectedStation)] = calibration;
          renderCalibrationSummary();
          renderTraitGroups();
        }
  ...
  function initApp() {
    ...
    Promise.all([
      ...
      apiFetch('/api/admin/durur', { method: 'GET' }).catch(function () { return safeFetchJson('/data/durur.json'); }),
